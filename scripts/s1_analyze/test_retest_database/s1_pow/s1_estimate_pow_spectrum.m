...
% Paths
config.path.dataset    = '../../../../databases/test_retest_database';
config.path.clean_data = sprintf('%s/derivatives',config.path.dataset);
...
for itester = 1 : numel(testers)
    ...
    for ifile = 1 : numel(dataset)
        ...
        path_pow = sprintf('%s/derivatives/%s/pow',config.path.dataset,...
            current_tester.name);
        ...
        if ~exist(outfile) || (exist(outfile) && config.overwrite)
            ...
            for itrial = 1 : size(data_matrix,3)
                ...
                channels_included_mask = false(size(complete_channel_labels));
                for ichannel = 1 : numel(data.label)
                    ...
                    if ~isempty(current_channel_index)
                        channels_included_mask(current_channel_index) = true;
            ...
            pow.channels_included_mask = channels_included_mask;
