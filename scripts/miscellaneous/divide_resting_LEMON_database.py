...
for current_sub in sub:
    ...
    for current_file in files:
        ...
        try:
            ...
            # Read the file
            raw = mne.io.read_raw(current_file,preload=True)
            ...
            # Export
            filename = current_sub + '_ses-1' + '_task-EO_eeg.set'
            out_filepath = os.path.join(config['path']['sub'],'sourcedata',filename)
            ...
            # Export
            filename = current_sub + '_ses-1' + '_task-EC_eeg.set'
            out_filepath = os.path.join(config['path']['sub'],'sourcedata',filename)
            export_mne_epochs(epochs,out_filepath)
